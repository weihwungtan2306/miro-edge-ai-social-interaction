...
MODEL_PATH = "/root/ssd_mobilenet_v2_face_quant_postprocess.tflite"
CONF_THRESHOLD = 0.5
SAVE_EVERY_N = 30
LOG_EVERY_N = 5
...
def callback(msg):
    ...
    if frame_count % LOG_EVERY_N == 0:
        rospy.loginfo(f"Frame {frame_count}: {len(faces)} face(s) | pipeline FPS: {avg_fps:.1f} (CPU-only)")
    ...
    if frame_count % SAVE_EVERY_N == 0:
        ...
        for score, box in faces:
            ...
            draw.text((left, max(0, top - 12)), f"{score:.2f}", fill="blue")
        out_path = f"/root/live_frame_cpu_{frame_count}.jpg"
        draw_img.save(out_path)
        rospy.loginfo(f"Saved snapshot: {out_path}")

rospy.init_node('cpu_face_detector', anonymous=True)
rospy.Subscriber('/miro/sensors/caml/compressed', CompressedImage, callback, queue_size=1, buff_size=2**16)
rospy.loginfo("CPU-only face detector node started, listening to /miro/sensors/caml/compressed...")
rospy.spin()
