...
CONF_THRESHOLD = 0.5
LOG_EVERY_N = 5
LOG_FILE = "/root/gaze_follow_log_v2.csv"

MAX_STEP_RAD = math.radians(5.0)
NO_FACE_RECENTER_FRAMES = 30
...
log_writer.writerow([
    "timestamp", "frame", "face_detected", "recentering", "cx", "cy",
...
def callback(msg):
    global frame_count, last_time, fps_times, current_yaw, current_pitch, no_face_counter
    ...
    if faces:
        ...
        no_face_counter += 1
        if no_face_counter > NO_FACE_RECENTER_FRAMES:
            recentering = 1
            current_yaw = step_toward(current_yaw, miro.constants.YAW_RAD_CALIB, MAX_STEP_RAD)
            current_pitch = step_toward(current_pitch, miro.constants.PITCH_RAD_CALIB, MAX_STEP_RAD)
    ...
    frame_count += 1
    log_writer.writerow([
        now, frame_count, int(bool(faces)), recentering,
    ...
    if frame_count % LOG_EVERY_N == 0:
        log_fh.flush()
        status = f"{len(faces)} face(s)" if faces else ("no face (recentering)" if recentering else "no face")
...
rospy.init_node('gaze_follow_kinematic_v2', anonymous=True)
rospy.on_shutdown(shutdown_hook)
rospy.Subscriber(topic_base_name + '/sensors/caml/compressed', CompressedImage, callback, queue_size=1, buff_size=2**16)
rospy.loginfo("Gaze-follow v2 (with auto-recenter) started, listening to " + topic_base_name + "/sensors/caml/compressed...")
rospy.spin()
