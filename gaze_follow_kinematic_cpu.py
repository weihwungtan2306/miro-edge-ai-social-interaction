...
CONF_THRESHOLD = 0.5
LOG_EVERY_N = 5
LOG_FILE = "/root/gaze_follow_log_cpu.csv"
...
def callback(msg):
    ...
    if frame_count % LOG_EVERY_N == 0:
        ...
        rospy.loginfo(f"Frame {frame_count}: {status} (CPU-only) | yaw={math.degrees(current_yaw):.1f} deg | pitch={math.degrees(current_pitch):.1f} deg | FPS: {avg_fps:.1f}")

def shutdown_hook():
    publish_kinematic(miro.constants.YAW_RAD_CALIB, miro.constants.PITCH_RAD_CALIB)
    log_fh.close()

rospy.init_node('gaze_follow_kinematic_cpu', anonymous=True)
rospy.on_shutdown(shutdown_hook)
rospy.Subscriber(topic_base_name + '/sensors/caml/compressed', CompressedImage, callback, queue_size=1, buff_size=2**16)
rospy.loginfo("Gaze-follow (kinematic, CPU-only) node started, listening to " + topic_base_name + "/sensors/caml/compressed...")
rospy.spin()
